\documentclass[a4paper,10pt,oneside,onecolumn,preprint]{elsarticle}
...
These findings establish adaptive thresholding as a practical, parameter-free alternative to manual WMC tuning for overlapping cluster-based GCN training.
\end{abstract}
...
\begin{enumerate}
  \item We formalize the connection between community-membership ambiguity and overlap assignment quality, showing that entropy and margin measures capture complementary aspects of structural ambiguity.
  \item We propose two adaptive overlap selection methods---Entropy-Adaptive WMC and Margin-Adaptive WMC---that replace the global WMC threshold with node-specific thresholds, eliminating the need for per-dataset WMC tuning.
  \item We present a comprehensive empirical evaluation across six diverse graph datasets, demonstrating that adaptive thresholding achieves competitive performance while removing a key hyperparameter.
  \item We conduct a detailed ambiguity analysis using stratified experiments and correlation studies, providing evidence that membership ambiguity is a principled criterion for overlap assignment.
\end{enumerate}
...
Figure~\ref{fig:ambiguity-stats} visualizes these characteristics.

\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{figures/dataset_ambiguity_stats.png}
\caption{Dataset ambiguity characteristics. (Left) Mean entropy, overlap ratio, and cluster count per dataset. IMDB stands out with the highest entropy and overlap ratio. (Center) Entropy standard deviation. (Right) Entropy distribution box plots. IMDB shows the widest distribution, explaining why it benefits most from adaptive thresholding.}
\label{fig:ambiguity-stats}
\end{figure}
...
Figure~\ref{fig:gain-variance} shows the relationship between entropy variance and performance gain, while Figure~\ref{fig:entropy-histograms} shows the entropy distributions across all six datasets, illustrating the variance differences.
...
\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{figures/entropy_histograms.png}
\caption{Membership entropy distributions for each dataset. IMDB shows the widest distribution (highest variance), explaining why it benefits most from adaptive thresholding. CiteSeer shows the narrowest distribution concentrated near zero.}
\label{fig:entropy-histograms}
\end{figure}

Ceiling effects (ACM) and very low ambiguity (CiteSeer) limit gains regardless of variance.
Figure~\ref{fig:ambiguity-histograms} shows the ambiguity score distributions, confirming that most nodes on CiteSeer and PubMed have low ambiguity, while IMDB shows a broader spread.

\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{figures/ambiguity_histograms.png}
\caption{Ambiguity score distributions ($A(i) = 1 - (p_1 - p_2)$) for each dataset. IMDB shows a more spread distribution with many nodes having moderate-to-high ambiguity, while CiteSeer and PubMed are concentrated near zero.}
\label{fig:ambiguity-histograms}
\end{figure}
...
The primary practical contribution is the elimination of the WMC hyperparameter.
...
We do not claim universal superiority over well-tuned fixed baselines---the contribution is principled threshold selection, not raw performance.
...
\begin{enumerate}
  \item \textbf{Default configuration:} Entropy-Adaptive with $\mathrm{WMC}_{\mathrm{base}} = 0.3$ and $\lambda = 0.50$. This provides competitive performance across all tested datasets without per-dataset tuning.
...
Both methods eliminate the need for manual WMC tuning while achieving competitive performance across six benchmark graphs.
...
The practical value of adaptive thresholding is the removal of the WMC hyperparameter, a parameter that varies across datasets and requires costly search.
