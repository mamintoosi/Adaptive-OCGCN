...
\begin{highlights}
\item We introduce Adaptive-OCGCN, an ambiguity-aware overlap assignment strategy for scalable graph neural networks.
\item Node-specific overlap decisions are derived from community-membership uncertainty using entropy and margin criteria.
\item The proposed method eliminates dataset-specific WMC tuning while remaining statistically competitive with tuned baselines on six benchmarks.
\item Margin-Adaptive WMC outperforms the fixed default threshold on five of six datasets, with a significant pooled gain ($p=0.021$).
\item Ambiguity analysis reveals that high-entropy nodes are 41--95\% overlap candidates, validating uncertainty-driven assignment.
\end{highlights}

\begin{abstract}
Graph Convolutional Networks (GCNs) have demonstrated strong performance on graph learning tasks, but their scalability remains challenging for large-scale graphs.
...
Across 20 seeds and six datasets, Margin-Adaptive WMC outperforms the untuned default threshold on five of six datasets, with a significant pooled paired gain ($p=0.021$ across 120 paired runs; aggregate across datasets $p=0.058$), and adaptive methods also improve performance on five of six datasets at comparable overlap ratios.
...
However, applying GCNs to large-scale graphs remains challenging due to neighborhood explosion, where each convolutional layer requires aggregating information from increasingly large neighborhoods, resulting in substantial memory and computational costs~\cite{Cluster-GCN}.
To address this issue, Cluster-GCN partitions a large graph into smaller subgraphs and performs mini-batch training on these clusters, achieving improved scalability while preserving most local graph structures~\cite{Cluster-GCN}.
...
Overlapping Cluster-GCN (OCGCN), introduced in our earlier study~\cite{Amintoosi2021}, was an initial attempt to incorporate overlapping community membership into partition-based GCN training.
...
In this work, we introduce \textbf{Adaptive-OCGCN}, an ambiguity-aware overlap assignment framework that replaces the fixed global overlap criterion with node-specific adaptive decisions.
...
We propose three complementary adaptive strategies: \textbf{Entropy-Adaptive WMC}, which uses normalized membership entropy to identify nodes with distributed community affiliations, \textbf{Margin-Adaptive WMC}, which exploits the gap between the two largest membership probabilities to detect nodes located near community boundaries, and \textbf{Hybrid-Adaptive WMC}, which combines both signals.
...
\begin{enumerate}
  ...
  \item We propose \textbf{Adaptive-OCGCN}, an ambiguity-aware overlap assignment framework with three adaptive strategies, Entropy-Adaptive WMC, Margin-Adaptive WMC, and their Hybrid combination, which derive node-specific overlap decisions from membership uncertainty.
...
Graph Convolutional Networks (GCNs) have become a fundamental approach for representation learning on graph-structured data by iteratively aggregating information from neighboring nodes~\cite{kipf2017semi}.
...
GraphSAGE~\cite{GraphSAGE} and FastGCN~\cite{chen2018fastgcn} reduce computational costs through neighborhood sampling, while VR-GCN~\cite{dai2018learning} introduces variance reduction techniques by reusing historical node representations.
...
Cluster-GCN~\cite{Cluster-GCN} follows a different strategy by partitioning a large graph into smaller clusters and performing mini-batch training on cluster-induced subgraphs.
...
Existing overlapping community detection methods include clique-based approaches, label propagation techniques, and matrix factorization based methods~\cite{Shang2014}.
Non-negative matrix factorization (NMF) methods represent each node using a membership vector over latent communities.
Deep Adversarial Non-negative Matrix Factorization (DANMF)~\cite{DANMF} extends this idea by combining deep autoencoding and adversarial learning to obtain informative community representations.

Recent studies have further explored graph neural network based approaches for overlapping community discovery.
He et al.~\cite{He2022} proposed a semi-supervised overlapping community detection framework using graph convolutional autoencoders.
Yuan et al.~\cite{Yuan2022} investigated overlapping community detection using graph convolutional networks on complex networks.
UCoDe~\cite{UCoDe2023} introduced a unified graph convolutional framework for community detection, while Li et al.~\cite{Li2023} proposed a dual graph neural network architecture for overlapping community detection.
Sismanis et al.~\cite{Sismanis2024} further explored graph attention networks for identifying overlapping communities.
...
While most previous studies have investigated overlapping communities for network analysis and community discovery, Overlapping Cluster-GCN (OCGCN)~\cite{Amintoosi2021} introduced a different perspective by utilizing overlapping memberships to improve scalable graph neural network training.
...
Graph Attention Networks (GAT)~\cite{vel2018graph}, for example, learn node-specific importance coefficients to dynamically control information aggregation.
Similarly, adaptive neighborhood selection approaches adjust message-passing structures according to graph topology and learning objectives~\cite{Peng2021, Guang2024}.
...
Gawlikowski et al.~\cite{Gawlikowski2023} provided a comprehensive survey of uncertainty estimation methods in deep neural networks, highlighting the importance of uncertainty modeling for reliable prediction.
Several graph learning studies have also incorporated confidence-aware mechanisms for improving robustness and reliability~\cite{Kwon2022, Park2023}.
...
Cluster-GCN~\cite{Cluster-GCN} accelerates GCN training by exploiting graph partitioning.
...
Overlapping Cluster-GCN (OCGCN)~\cite{Amintoosi2021} extends Cluster-GCN by relaxing the assumption of disjoint graph partitions.
...
\item \textbf{Soft Community Assignment.}
...
\item \textbf{Overlapping Subgraph Construction.}
...
DANMF~\cite{DANMF} is a deep adversarial non-negative matrix factorization method that learns latent community representations.
...
Here, $\mathcal{C}_i$ represents the set of clusters to which node $i$ is assigned.

Although effective, this formulation applies the same $\mathrm{WMC}$ value to all nodes.
Consequently, overlap assignment does not consider the confidence of individual community memberships.
...
Therefore, the problem addressed in this work is formulated as follows:

\textbf{Given a soft membership matrix $P$, determine node-specific overlap assignments by exploiting membership ambiguity, while preserving the scalability advantages of partition-based GNN training.}
...
The desired adaptive assignment mechanism should satisfy three properties:

\begin{enumerate}
    \item \textbf{Ambiguity awareness:}
    Nodes with uncertain community memberships should have higher probability of being assigned to multiple clusters.

    \item \textbf{Confidence preservation:}
    Nodes with dominant memberships should avoid unnecessary overlap.

    \item \textbf{Hyperparameter efficiency:}
    The method should reduce dependence on dataset-specific threshold tuning.
\end{enumerate}
%%======================================================================
...
\begin{equation}
a_i = \frac{H_{\mathrm{norm}}(i) + A(i)}{2},
...
\mathrm{WMC}_i =
\mathrm{WMC}_{\mathrm{base}}
\left(1-\lambda \cdot \frac{H_{\mathrm{norm}}(i) + A(i)}{2}\right).
\label{eq:hybrid-adaptive}
\end{equation}

By averaging the two normalized ambiguity scores, the hybrid exploits both global distribution spread and top-two competition, stays in $[0,1]$, and remains comparable across datasets.

\subsection{Comparison of Adaptive Methods}
\label{subsec:comparison}

Table~\ref{tab:method-comparison} summarizes the characteristics of the two proposed strategies.
...
Per-node computation & $O(K)$ & $O(K)$ & $O(K)$ \\
...
\begin{enumerate}
  ...
  \item \textbf{OCGCN with fixed WMC:} The original OCGCN~\cite{Amintoosi2021} with fixed WMC thresholds of 0.10, 0.20, 0.30, 0.40, and 0.50, representing the upper bound when hyperparameters are tuned.
  \item \textbf{Adaptive WMC variants:} Entropy-Adaptive, Margin-Adaptive, and their Hybrid combination with $\lambda \in \{0.25, 0.50, 0.75\}$.
\end{enumerate}
...
\midrule
Cora      & 0.798~$\pm$~0.051 & \textbf{0.832~$\pm$~0.020} & 0.828~$\pm$~0.029 & 0.830~$\pm$~0.025 & 0.829~$\pm$~0.030 \\
CiteSeer  & 0.705~$\pm$~0.053 & 0.736~$\pm$~0.030 & 0.727~$\pm$~0.045 & \textbf{0.736~$\pm$~0.021} & 0.729~$\pm$~0.050 \\
PubMed    & 0.521~$\pm$~0.143 & 0.612~$\pm$~0.122 & 0.580~$\pm$~0.136 & \textbf{0.629~$\pm$~0.127} & 0.622~$\pm$~0.133 \\
ACM       & \textbf{0.911~$\pm$~0.035} & 0.879~$\pm$~0.071 & 0.881~$\pm$~0.080 & 0.899~$\pm$~0.069 & 0.896~$\pm$~0.072 \\
DBLP      & 0.829~$\pm$~0.014 & 0.844~$\pm$~0.017 & 0.846~$\pm$~0.024 & 0.851~$\pm$~0.013 & \textbf{0.856~$\pm$~0.012} \\
IMDB      & 0.393~$\pm$~0.012 & 0.429~$\pm$~0.015 & 0.435~$\pm$~0.010 & \textbf{0.443~$\pm$~0.012} & 0.438~$\pm$~0.012 \\
\bottomrule
\end{tabular}
\end{table}
...
Margin-Adaptive outperforms the untuned default WMC~=~0.30 on five of six datasets, with the largest gains on ACM (+2.0 F1), PubMed (+1.6 F1), and IMDB (+1.4 F1).
...
IMDB      & 0.433 & 0.440 & \textbf{0.443} & 0.75 \\
\bottomrule
\end{tabular}
\end{table}
...
DBLP      & \textbf{0.856} & 0.851 & 0.852 & 0.25 \\
IMDB      & 0.435 & 0.441 & \textbf{0.447} & 0.75 \\
\bottomrule
\end{tabular}
\end{table}
...
DBLP      & 0.858 & \textbf{0.863} & 0.853 & 0.50 \\
...
\midrule
Cora    & hybrid & 1.44 & WMC=0.20 & 1.48 & +0.005 & Adaptive \\
...
CiteSeer & margin & 1.18 & WMC=0.30 & 1.15 & +0.000 & Adaptive \\
IMDB    & margin & 2.53 & WMC=0.20 & 2.56 & +0.003 & Adaptive \\
\bottomrule
\end{tabular}
\end{table}

At equal overlap ratios, adaptive methods win on five of six datasets, with gains ranging from +0.000 (CiteSeer, a near tie) to +0.018 (ACM).
...
\caption{Fair comparison at matched overlap ratios (micro F1). Green annotations indicate adaptive wins; red indicates fixed wins. Adaptive methods win on five of six datasets, with the largest gain on ACM (+0.018).}
...
\midrule
WMC=0.10 & 0.080 & 0.049 & 0.349 & $-$0.056 & 0.095 & 0.023 \\
WMC=0.20 & 0.097 & 0.030 & 0.594 & $-$0.102 & 0.102 & 0.021 \\
WMC=0.30 & 0.162 & 0.056 & 0.520 & $-$0.124 & 0.196 & 0.025 \\
\textbf{Entropy} & 0.105 & 0.040 & 0.280 & $-$0.127 & 0.099 & 0.018 \\
\textbf{Margin} & 0.111 & 0.058 & 0.498 & $-$0.052 & 0.172 & 0.027 \\
\textbf{Hybrid} & 0.114 & 0.077 & 0.463 & $-$0.062 & 0.138 & 0.018 \\
\bottomrule
\end{tabular}
\end{table}

The fixed WMC~=~0.30 threshold achieves the highest efficiency on the citation networks (Cora 0.162, CiteSeer 0.196, PubMed 0.520) because it attains large F1 gains with modest overlap.
Among adaptive methods, Margin-Adaptive is the most efficient on PubMed (0.498) and IMDB (0.027), and Hybrid-Adaptive on DBLP (0.077) and Cora (0.114).
...
IMDB     & margin & WMC=0.30 & 3.307 & 0.004 & 0.004 & Adaptive \\
IMDB     & hybrid & WMC=0.30 & 2.136 & 0.046 & 0.070 & Adaptive \\
PubMed   & margin & WMC=0.30 & 1.025 & 0.318 & 0.452 & No diff \\
ACM      & margin & WMC=0.30 & 1.425 & 0.170 & 0.189 & No diff \\
Cora     & margin & WMC=0.30 & $-$0.334 & 0.742 & 0.784 & No diff \\
CiteSeer & margin & WMC=0.30 & 0.026 & 0.980 & 0.869 & No diff \\
\midrule
Pooled (120) & margin & WMC=0.30 & 2.333 & 0.021 & 0.019 & Adaptive \\
Pooled (120) & hybrid & WMC=0.30 & 1.778 & 0.078 & 0.042 & Adaptive (Wilcoxon) \\
\bottomrule
\end{tabular}
\end{table}

Against the untuned default WMC~=~0.30, Hybrid-Adaptive is significantly better on DBLP ($p = 0.020$) and Margin-Adaptive on IMDB ($p = 0.004$).
Aggregating the six per-dataset mean differences, the gain of Margin-Adaptive over the default is marginally significant ($t = 2.45$, $p = 0.058$).
Pooled across all 120 paired runs (20 seeds $\times$ 6 datasets), it is statistically significantly better than the default (paired $t$-test $t = 2.33$, $p = 0.021$; Wilcoxon $p = 0.019$).
...
The relationship is not strictly monotonic---ceiling effects on ACM and the near-optimal default on Cora confound it---but high entropy variance is a useful indicator that adaptive methods can help.
...
Margin-Adaptive (or the Hybrid) with $\lambda = 0.50$ achieves competitive performance across all six datasets without per-dataset tuning, and is significantly better than the fixed default in the pooled paired analysis ($p=0.021$).
...
At matched overlap ratios, adaptive methods improve by +0.000 to +0.018 F1 over fixed baselines on five of six datasets.
...
\begin{enumerate}
  \item \textbf{Default configuration:} Margin-Adaptive (or the Hybrid) with $\mathrm{WMC}_{\mathrm{base}} = 0.3$ and $\lambda = 0.50$. This single configuration is significantly better than the fixed default in the pooled paired analysis ($p=0.021$), outperforms it on five of six datasets, and provides competitive performance across all tested datasets without per-dataset tuning.
...
With 20 seeds per configuration in the main comparison (10 in the ablation), we detected a significant pooled improvement of Margin-Adaptive over the default threshold ($p = 0.021$).
...
All three replace per-dataset WMC grid search with a single universal configuration ($\mathrm{WMC}_{\mathrm{base}} = 0.3$, $\lambda = 0.5$) that achieves competitive performance across six benchmark graphs; Margin-Adaptive is significantly better than the fixed default in the pooled paired analysis ($p=0.021$).
...
Performance gains from adaptive thresholding are modest but real: at matched overlap ratios, adaptive methods improve by +0.000 to +0.018 F1 over fixed baselines on five of six datasets, and Margin-Adaptive is significantly better than the fixed default in the pooled paired analysis ($p=0.021$).
...
\bibliographystyle{plainnat}%{apa-good}%{amsplain}%
\bibliography{References}%\bibliography{bibsample}%
