...
\item Margin-Adaptive WMC outperforms the fixed default threshold on four of six datasets, with a significant pooled gain ($p=0.022$).
...
Across 20 seeds and six datasets, Margin-Adaptive WMC outperforms the untuned default threshold on four of six datasets, with a significant pooled paired gain ($p=0.022$ across 120 paired runs; aggregate across datasets $p=0.061$), and adaptive methods also improve performance on four of six datasets at comparable overlap ratios.
...
Per-node computation & $O(K)$ & $O(K \log K)$ & $O(K)$ \\
...
\subsection{Training Time and Computational Overhead}
...
At matched overlap ratios, adaptive methods improve by +0.000 to +0.018 F1 over fixed baselines on four of six datasets.
...
Performance gains from adaptive thresholding are modest but real: at matched overlap ratios, adaptive methods improve by +0.000 to +0.018 F1 over fixed baselines on four of six datasets, and Margin-Adaptive is significantly better than the fixed default in the pooled paired analysis ($p=0.022$).
