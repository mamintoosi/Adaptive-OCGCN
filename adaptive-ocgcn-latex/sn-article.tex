...
\caption{
  Conceptual overview of the adaptive overlap assignment framework. 
  (a) Conventional fixed-threshold approach applies a uniform global rule, where boundary nodes (e.g., node~5) are assigned to a single cluster, discarding cross-cluster connections. 
  (b) The proposed node-specific adaptive mechanism leverages membership ambiguity to set per-node thresholds, allowing highly ambiguous nodes to participate in multiple clusters and preserve cross-cluster information. 
  (c) Schematic illustration contrasting the two strategies, highlighting the conceptual benefit of adaptive thresholding.
}
\label{fig:framework}
\end{figure}

In this work, we introduce \textit{Adaptive-OCGCN}, an ambiguity-aware overlap assignment framework that replaces the fixed global overlap criterion with node-specific adaptive decisions (Figure \ref{fig:framework}).
...
This section reviews the relevant literature in two parts. 
First, we survey the broad research areas that contextualize our work: scalable graph neural network training (Section~\ref{subsec:scalable-gnn}), overlapping community detection and soft membership modeling (Section~\ref{subsec:overlap-community}), overlap-aware partition-based GNN training (Section~\ref{subsec:overlap-aware}), and adaptive and uncertainty-aware graph learning (Section~\ref{subsec:adaptive-uncertainty}). 
Second, we provide detailed technical background on the specific methods that directly underpin our proposed approach: Cluster-GCN (Section~\ref{subsec:clustergcn}), Overlapping Cluster-GCN (Section~\ref{subsec:ocgcn}), soft community detection via DANMF (Section~\ref{subsec:danmf}), and community membership ambiguity measures (Section~\ref{subsec:ambiguity}). 
This organization guides the reader from the general landscape to the specific gaps that motivate our ambiguity-based adaptive thresholding framework.

\subsection{Scalable Graph Neural Network Training}
\label{subsec:scalable-gnn}
...
Beyond scalability, several studies have sought to improve GCN classification accuracy through ensemble strategies; for instance, Bo and Yu~\citep{Bo2025} proposed a multi-classifier fusion approach that aggregates predictions from multiple GCN models using fuzzy integrals, achieving consistent gains on citation network benchmarks.
...
\label{subsec:overlap-community}
...
\label{subsec:adaptive-uncertainty}
...
Recent work has further extended adaptive mechanisms to heterogeneous and safety-critical settings; Khaledian and Falahati~\citep{Khaledian2026} employed a DDI-aware cross-attention mechanism to fuse GraphSAGE embeddings with pharmacological safety signals, demonstrating that attention-based adaptivity can effectively balance predictive accuracy and constraint satisfaction in graph-based recommendation systems.
...
All experiments follow the same protocol, which we first describe below, and then report the quantitative findings.
...
The remainder of this subsection presents the detailed experimental findings. 
We first compare adaptive methods against fixed-threshold baselines, then examine the sensitivity to adaptation strength via ablation, ensure a fair comparison by matching overlap ratios, analyze overlap efficiency, report computational overhead, assess statistical significance, and finally provide an in-depth ambiguity analysis to validate our core hypothesis.
...
Margin-Adaptive outperforms the untuned default WMC~=~0.30 on four of six datasets, with the largest gains on ACM (+2.0 F1, i.e. +2.0 percentage points), PubMed (+1.6 F1), and IMDB (+1.4 F1).
...
This subsection provides an in-depth analysis of the relationship between node-level membership ambiguity and overlap assignment.
Our central hypothesis is that nodes with uncertain community memberships are more suitable candidates for overlap than nodes with confident assignments.
To test this hypothesis, we conduct three complementary analyses. 
First, we present summary statistics of ambiguity measures across all datasets. % (Section~4.3.7.1). 
Second, we examine Spearman rank correlations between ambiguity indicators and overlap properties. %(Section~4.3.7.2). 
Third, we stratify nodes by entropy terciles to directly compare overlap rates across ambiguity levels. %(Section~4.3.7.3). 
Finally, we investigate why adaptive thresholding improves performance more substantially on some datasets than others. % (Section~4.3.7.4).

\paragraph{Dataset Ambiguity Statistics.}
\label{sec:amb-stats}

Table~\ref{tab:ambiguity-stats} reports the mean and standard deviation of normalized entropy and margin-based ambiguity, along with the observed overlap ratio and average number of clusters per node for each dataset, all derived from the DANMF membership matrix under the fixed WMC=0.30 overlap criterion.

\begin{table}[ht]
\centering
\caption{Summary statistics of membership ambiguity and overlap behavior across datasets.}
...
First, IMDB exhibits by far the highest mean entropy (0.419) and entropy standard deviation (0.280), as well as the largest overlap ratio (0.621) and mean clusters per node (2.20). 
This reflects the dense, role-heterogeneous nature of IMDB, where many nodes naturally straddle multiple communities. 
Second, CiteSeer has the lowest mean entropy (0.071) and the smallest standard deviation (0.126), indicating that most nodes have near-deterministic community memberships with very little ambiguity; accordingly, its overlap ratio is only 0.137. 
These statistics suggest that adaptive thresholding is most likely to provide benefits on datasets with high entropy variance, where there is room to differentiate between confident and ambiguous nodes, while its impact may be limited on datasets with near-uniform low entropy.

\paragraph{Correlation Between Ambiguity and Overlap.}

To quantify the strength of the relationship between ambiguity measures and overlap behavior, Table~\ref{tab:correlations} reports Spearman rank correlations between entropy, margin-based ambiguity, overlap ratio, and the number of assigned clusters. 
All reported correlations are statistically significant at $p < 0.001$.

\begin{table}[ht]
\centering
\caption{Spearman rank correlations between ambiguity measures and overlap indicators.}
...
Entropy correlates strongly with both overlap ratio and cluster count across all datasets, with coefficients ranging from 0.60 to 0.81. 
This is a robust confirmation that high-entropy nodes are systematically assigned to more clusters, supporting our central hypothesis that ambiguity is a meaningful criterion for overlap selection.
In contrast, margin-based ambiguity shows strong correlations with overlap on four of the six datasets (Cora, PubMed, ACM, IMDB), but weak correlations on CiteSeer and DBLP ($r \approx 0.10$). 
The weak correlation on these two datasets is explained by the near-zero or negative correlation between entropy and ambiguity ($r = -0.366$ for CiteSeer and $r = -0.398$ for DBLP), indicating that the two measures capture fundamentally different aspects of the membership distribution: entropy reflects spread across all communities, while margin-based ambiguity focuses specifically on the competition between the top two communities. 
On PubMed, the two measures are nearly identical ($r = 0.981$), suggesting redundancy; on this dataset, using either entropy or margin alone would yield almost the same overlap assignments.

\paragraph{Stratified Ambiguity Analysis.}

To directly test the hypothesis that high-ambiguity nodes are more likely to be suitable for overlap, we stratify nodes in each dataset into entropy terciles (low, medium, high) and compare their overlap assignment rates.
Table~\ref{tab:stratified} presents the overlap ratio and mean number of assigned clusters for each entropy tercile.

\begin{table}[ht]
\centering
\caption{Overlap ratio and average cluster count per entropy tercile.}
...
The results are remarkably consistent across all datasets. 
High-entropy nodes receive overlap assignments in 41--95\% of cases, whereas low-entropy nodes receive overlap in only 0--17\% of cases. 
On IMDB, the most ambiguous nodes are assigned to an average of 3.4 clusters, confirming that they genuinely straddle multiple communities. 
On CiteSeer and DBLP, many nodes have near-zero entropy, and the two lowest terciles merge into a single low-entropy bin with almost no overlap. 
Figure~\ref{fig:overlap-by-tercile} visualises the stark contrast between entropy terciles, providing compelling visual evidence that entropy is a strong predictor of overlap suitability.

\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{figures/overlap_vs_entropy.pdf}
\caption{
  Overlap ratio by entropy tercile for each dataset. 
  Bars are ordered from low to high entropy (left to right). 
  For CiteSeer and DBLP, the two lowest terciles are merged into a single bar (Low+Med) due to near-zero overlap in low-entropy nodes. 
  The pattern confirms that membership ambiguity is a reliable indicator of overlap suitability.}
...
Finally, we examine the relationship between entropy statistics and the performance gain achieved by adaptive thresholding.
Table~\ref{tab:gains-vs-variance} compares each dataset's entropy variance with the F1 improvement (in percentage points) of the best adaptive method over the fixed WMC=0.30 baseline.

\begin{table}[ht]
\centering
\caption{Relationship between entropy variance and adaptive performance gain.}
\label{tab:gains-vs-variance}
\begin{tabular}{l c c l} %p{6cm}
\toprule
\textbf{Dataset} & \textbf{\shortstack{Entropy\\Variance}} & \textbf{\shortstack{Gain vs. Default\\(F1 pts)}} & \textbf{Interpretation} \\
\midrule
...
Performance gains show a partial positive relationship with entropy variance: the largest gains occur on datasets with higher variance (ACM, PubMed, IMDB), while CiteSeer, with the lowest variance, gains nothing.
However, the relationship is not strictly monotonic. 
Cora, for example, has variance 0.028—similar to that of PubMed and DBLP—but shows no gain because WMC=0.30 is already the best fixed threshold (0.832 micro F1), leaving no room for improvement regardless of method. 
In contrast, PubMed and DBLP achieve gains of +1.6 and +1.2 F1, respectively, even though their entropy variances are comparable to Cora, because their default WMC=0.30 underperforms the per-dataset optimum (WMC=0.20 for PubMed, WMC=0.10 for DBLP). 
Adaptive methods are able to lower the threshold for ambiguous nodes and thus recover the lost performance.
The dashed trend line in Figure~\ref{fig:gain-variance} shows the linear fit between entropy variance and performance gain, illustrating the overall positive association despite the outliers (Cora and IMDB). 
Ceiling effects (ACM), near-optimal defaults (Cora), and very low ambiguity (CiteSeer) all limit the practical gain, regardless of variance.

\begin{figure}[ht]
\centering
\includegraphics[width=0.75\textwidth]{figures/gain_vs_variance.png}
\caption{Performance gain (micro-F1 improvement over WMC=0.30) versus entropy variance ($\sigma^2$) for each dataset. Datasets with higher entropy variance (ACM, PubMed, IMDB) generally show larger gains, while CiteSeer, with the lowest variance, shows no gain. The dashed line indicates the overall linear trend.}
...
We first validate the ambiguity hypothesis and examine the nature of performance gains (Section~\ref{subsec:interpretation}). 
We then discuss the practical benefits of eliminating the WMC hyperparameter and provide recommendations (Section~\ref{subsec:practical}). 
The agnosticism of our method to the underlying community detection algorithm is discussed in Section~\ref{subsec:agnosticism}. 
Threats to validity are addressed in Section~\ref{sec:threats}, followed by concluding remarks in Section~\ref{sec:conclusion}.
...
High-entropy nodes receive overlap assignments in 41--95\% of cases and are assigned to 1.5--3.4 clusters on average, confirming that they genuinely straddle multiple communities. 
Low-entropy nodes, by contrast, receive overlap in only 0--17\% of cases and are well-served by single-cluster training. 
This pattern is consistent across all datasets and provides compelling empirical support for our central hypothesis.

At matched overlap ratios, adaptive methods improve performance by +0.003 to +0.018 F1 over fixed baselines on four of six datasets. 
The gains are modest but real, reflecting the fact that the original WMC threshold already captures a substantial portion of the useful overlap signal. 
...
The contribution is not universal superiority over well-tuned baselines, but rather principled threshold selection that eliminates the tuning cost.
...
Margin-Adaptive performs best on datasets where ambiguity is primarily a top-two competition (DBLP: $\lambda=0.25$; CiteSeer: $\lambda=0.50$). 
...
\subsection{Practical Benefits and Recommendations}
\label{subsec:practical}

The primary practical contribution of this work is the elimination of the WMC hyperparameter search. 
Our fixed-WMC comparison shows that the optimal WMC varies substantially across datasets: WMC=0.30 for Cora and CiteSeer, WMC=0.20 for PubMed, WMC=0.10 for DBLP and IMDB, and no overlap for ACM. 
...
Importantly, this is not a claim of universal superiority over well-tuned baselines, but rather a demonstration that a single universal configuration ($\mathrm{WMC}_{\mathrm{base}}=0.3$, $\lambda=0.5$) performs comparably to per-dataset tuned WMC. 
While $\lambda$ is technically a hyperparameter, it acts as a global robustness knob rather than a dataset-specific threshold. 
As shown in the ablation study, setting $\lambda=0.50$ consistently yields near-optimal results across all datasets, with sensitivity substantially lower than that of the fixed WMC baseline.

Based on these findings, we recommend:

\begin{enumerate}
  \item {Default configuration}: Margin-Adaptive (or the Hybrid) with $\mathrm{WMC}_{\mathrm{base}}=0.3$ and $\lambda=0.50$. 
...
The largest graph, PubMed, has 19,717 nodes, which is relatively small by modern GNN standards. 
However, it is crucial to distinguish between the scalability of the proposed adaptive thresholding mechanism and that of the upstream clustering algorithm. 
...
The current evaluation is limited to standard benchmark sizes because the upstream dependency—specifically the DANMF clustering algorithm—becomes a computational bottleneck on graphs with millions of nodes. 
Scaling to larger graphs (e.g., OGB benchmarks) would further strengthen the claims and test whether the ambiguity signal remains informative at scale.
...
On PubMed, DANMF yields a highly imbalanced partition (the largest cluster contains about 62\% of the nodes), which degrades the cluster-based no-overlap baseline (0.521 micro F1) below a random-partition control (0.852). 
The adaptive overlap methods still recover substantial accuracy (0.628) in this regime, suggesting that balancing cluster sizes is a promising avenue for future work.
