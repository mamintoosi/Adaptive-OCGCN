...
% BEGIN_REVIEW
% اگر این عبارت خط بعد که در پاراگراف قبل آمده است، ضروری نیست آن را حذف کن
%  ($p=0.022$ across 120 paired runs; aggregate across datasets $p=0.061$)
% END_REVIEW
...
\tableofcontents
% BEGIN_REVIEW
% مطابق فهرست مطالب الان ۱۱ بخش داریم که برای یک مقاله زیاد است. لطفا آنها را به حدود ۵ بخش کاهش بده. مثلا به نظرم بخش
% Background
%  می تواند در بخش قبل از خودش ادغام شود. یا بخش Problem Formulation  در بخش بعد خودش ادغام شود.
% بخش نتایج به عنوان زیربخشی از بخش قبل خودش باشد. بخشهای آنالیز و Ablation Study ادغام شوند.
%  فایل با پسوند toc  حاوی فهرست مطالب هست
% END_REVIEW
%%======================================================================
...
\textit{Given a soft membership matrix $P$, determine node-specific overlap assignments by exploiting membership ambiguity, while preserving the scalability advantages of partition-based GNN training.}
...
% BEGIN_REVIEW
% در جدول {tab:datasets} یک ستون با نام Type  داریم. اما به نظرم Heterogeneous در کنار/برابر  Citation  نیست. ACM  هم از نوع  Citation است. یا این انواع درست شوند یا این ستون حذف شود.
% END_REVIEW
...
% BEGIN_REVIEW
% به این امید که در جدول {tab:main-results} میانگین همه مجموعه دادگان برای یکی از روشهای پیشنهادی بهتر از دو روش اول باشد، میانگین ستونها را هم محاسبه و به عنوان سطر آخر اضافه کن
% END_REVIEW
...
% BEGIN_REVIEW
% شکل {fig:main-results} یک مشکل کوچک دارد: دور نمودار میله ای روش برنده در هر مجموعه داده یک کادر قرمز رنگ کشیده شده است که خوبه. مشکل این هست که در کادر راهنما (legend)  دور رنگ مربوط به روش برنده در اولین مجموعه داده کادر قرمز کشیده شده است. خط قرمز در این کادر راهنما باید حذف شود. اگر نمی شود، از راهی دیگر برای مشخص کردن روش برنده استفاده شود.
% END_REVIEW
...
% BEGIN_REVIEW
% در جدول این بخش ، برتری روشهای پیشنهادی نسبت به روش پایه به وصوح مشخص نیست. اگر واقعا روش پیشنهادی بهتر هست (مثلا میانگین کلی بهتری دارد) مشخص شود. وگرنه بهتر نیست این بخش حذف شود؟
% END_REVIEW	
...
% BEGIN_REVIEW
% در  جدول {tab:gains-vs-variance} گفته شده که چون {Entropy Variance} مربوط به Cora نزدیک به مقدار بهینه بوده:
 % no room regardless of method 
 % اولا مقدار بهینه از کجا آمد/چیست؟ دوما واریانس آنتروپی مجموعه دادگان PubMed, DBLP هم نزدیک ۰.۰۳ است - نزدیک به مقدار Cora- چرا اونها بهتر شدند؟
 
 % خط چین وسط شکل {fig:gain-variance} چی رو مشخص می کنه؟ نوشته شده `Trend'  اما به نظرم در متن هیچ توضیحی در مورد آن ذکر نشده است.
% END_REVIEW
...
% BEGIN_REVIEW
% در متن خط قبل نوشته شده از ``+0.000 to +0.018'' . از صفر که گفتن نداره دیگه.
% همین مورد در بخش نتیجه گیری هم هست.
% END_REVIEW
